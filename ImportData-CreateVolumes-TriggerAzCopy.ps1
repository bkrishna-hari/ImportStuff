...
.ASSETS 
    ...
    VolumeSize: The size of volume which will be in bytes
...
workflow ImportData-CreateVolumes-TriggerAzCopy
{
    ...
    $VolumeSize = Get-AutomationVariable –Name "VolumeSize" 
        ...
        InlineScript
        {
            ...
            foreach ($data in $ImportInfraList)
            {
                ...
                while ($AzCopyLoopIndex -lt $DriveList.Count)
                {
                    ...
                    Write-Output "  Drive: $DriveName    VolumeName: $VolumeName"
